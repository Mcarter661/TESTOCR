...
-   **Bank-Specific Parsers:** Dedicated parsers are implemented for PNC, Truist, Chase (including JPMorgan Commercial Checking), BofA, Wells Fargo, Citibank, US Bank, and Webster banks, along with a generic parser fallback. Bank detection uses a three-pass approach: letterhead (first 500 chars), header (first 1500 chars), then full-text, with Chase prioritized before BofA to avoid false matches from transaction descriptions mentioning other banks.
